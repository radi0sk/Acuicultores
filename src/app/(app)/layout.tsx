...
function AppLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const useDashboardLayout = dashboardLayoutPaths.some(p => pathname.startsWith(p));
  
  if (useDashboardLayout) {
    return (
        <div className="grid h-screen w-full md:grid-cols-[280px_1fr] lg:grid-cols-[320px_1fr]">
          <aside className="hidden h-full overflow-y-auto border-r bg-muted/40 md:block no-scrollbar">
              <div className="sticky top-0 p-6">
                  <DashboardNav />
              </div>
          </aside>
          <div className="flex flex-col">
            {/* Header for mobile and general top bar content */}
                      ...
                      <DropdownMenuLabel className="font-headline text-xs font-normal text-muted-foreground px-2">Cambiar Perfil</DropdownMenuLabel>
                       <DropdownMenuRadioGroup value={activeProfile} onValueChange={setActiveProfile}>
                          {userProfile?.roles?.map(role => (
                            <DropdownMenuRadioItem key={role} value={role} className="font-body">
                                {role}
