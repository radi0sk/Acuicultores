...
import type { EnhancePublicationOutput } from '@/ai/flows/enhance-publication-flow';
...
export default function CreatePublicationPage() {
    ...
    const addBlock = (type: 'text' | 'image' | 'video', url?: string) => {
        const newBlock: ContentBlock = { id: uuidv4(), type, value: url || '' };
    ...
    const handleSave = async (status: 'published' | 'draft') => {
        ...
        try {
            ...
            const storedBlocks: StoredContentBlock[] = blocks
                        ...
                        <CardContent className="space-y-4">
                            ...
                            {blocks.length === 0 && (
                                <Alert>
                                    ...
                                    <AlertDescription className="font-body">Usa los botones de abajo para añadir tu primer bloque de texto.</AlertDescription>
                                </Alert>
                            )}
                            ...
                            <Separator/>
                             ...
                             <div className="flex items-center gap-2">
                                ...
                                <CldUploadButton
                                    as="span"
                                    uploadPreset="imagen-publicacion"
                                    options={{
                                        folder: 'publications',
                                        sources: ['local', 'url'],
                                        resourceType: 'image'
                                    ...
                                    onSuccess={(result: any) => {
                                        if (result.info?.secure_url) {
                                            addBlock('image', result.info.secure_url);
                                        }
                                    }}
                                >
                                    <Button variant="outline" size="sm" as="span"><ImageIcon className="mr-2 h-4 w-4" /> Imagen</Button>
                                </CldUploadButton>
                                <CldUploadButton
                                    as="span"
                                    uploadPreset="imagen-publicacion"
                                    ...
                                    onSuccess={(result: any) => {
                                        if (result.info?.secure_url) {
                                            addBlock('video', result.info.secure_url);
                                        }
                                    }}
                                >
                                     <Button variant="outline" size="sm" as="span"><Video className="mr-2 h-4 w-4" /> Video</Button>
                                </CldUploadButton>
                            </div>
                            ...
                            <article className="bg-background rounded-lg shadow-md p-6 space-y-4">
                                ...
                                {aiSuggestion && (
                                     <Card className="absolute -top-4 -right-4 z-20 bg-primary/90 text-primary-foreground shadow-2xl border-2 border-primary w-full max-w-md">
                                        ...
                                        <CardContent>
                                            <p className="font-body text-sm">Hemos mejorado tu texto. ¿Quieres aplicar estos cambios?</p>
                                        </CardContent>
